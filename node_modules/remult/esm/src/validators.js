export class Validators {
    ...
    static in = createValueValidatorWithArgs((val, values) => values.includes(val), (values) => `Value must be one of ${values.join(', ')}`);
    static notNull = createValueValidator((val) => val != null, 'Should not be null');
    static enum = createValueValidatorWithArgs((value, enumObj) => Object.values(enumObj).includes(value), (enumObj) => `Value must be one of ${Object.values(enumObj)
        .filter((x) => typeof enumObj[x] !== 'number')
        .join(', ')}`);
...
export function createValueValidatorWithArgs(validate, defaultMessage) {
    const result = createValidatorWithArgs((_, e, args) => {
        if (e.value === undefined || e.value === null)
            return true;
